...
const MeetingCard: React.FC<{ meeting: Meeting }> = ({ meeting }) => {
    ...
    return (
        <Card className="flex flex-col justify-between">
            <div>
                <span className={`text-xs font-semibold px-2 py-1 rounded-full ${meeting.status === 'Scheduled' ? 'bg-blue-100 text-blue-800' : 'bg-slate-200 text-slate-700'}`}>
                    {meeting.status}
                </span>
                ...
                <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {attendeeNames.slice(0, 5).map(name => (
                            <span key={name} className="text-xs bg-slate-200 text-brand-text px-2 py-1 rounded-full">{name}</span>
                        ))}
                        {attendeeNames.length > 5 && <span className="text-xs bg-slate-200 text-brand-text px-2 py-1 rounded-full">+{attendeeNames.length - 5} more</span>}
                    </div>
                </div>
            </div>
