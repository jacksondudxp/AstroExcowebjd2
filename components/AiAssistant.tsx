...
const AiAssistant: React.FC = () => {
  ...
  return (
    <>
      {assistantButton}
      <div className="fixed bottom-28 right-8 w-full max-w-md h-[60vh] bg-slate-50 rounded-xl shadow-2xl flex flex-col z-40 animate-slide-in-up border border-slate-200">
        <header className="bg-white p-4 rounded-t-xl border-b border-slate-200">
          <h3 className="text-lg font-bold text-brand-text">AstroClub AI Assistant</h3>
        </header>
        <div ref={chatContainerRef} className="flex-1 p-4 overflow-y-auto space-y-4">
          {messages.map((msg, index) => (
            <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] p-3 rounded-2xl ${msg.role === 'user' ? 'bg-brand-accent text-white rounded-br-none' : 'bg-slate-200 text-brand-text rounded-bl-none'}`}>
          ...
          {isLoading && messages[messages.length-1].role === 'user' && (
             <div className="flex justify-start">
              <div className="max-w-[80%] p-3 rounded-2xl bg-slate-200 text-brand-text rounded-bl-none">
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-slate-200">
          <div className="flex items-center bg-white rounded-lg border border-slate-300">
            <input
              ...
              className="flex-1 bg-transparent p-3 text-brand-text focus:outline-none"
              disabled={isLoading}
            />
            <button type="submit" disabled={isLoading || !input.trim()} className="p-3 text-brand-accent disabled:text-brand-text-dark transition-colors">
