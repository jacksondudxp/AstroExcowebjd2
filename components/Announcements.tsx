...
const Announcements: React.FC = () => {
  ...
  const getTargetGroupPill = (targetGroup: AnnouncementTargetGroup) => {
    const styles = {
      'All Members': 'bg-blue-100 text-blue-800',
      'Current Committee': 'bg-green-100 text-green-800',
      'Past Committees': 'bg-purple-100 text-purple-800',
    };
    return (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Create New Announcement">
        <form onSubmit={handleSubmit} className="space-y-4">
          <input 
            ...
            required 
            className="w-full p-2 rounded-md bg-slate-100 border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"
          />
          <textarea 
            name="content" 
            placeholder="Content" 
