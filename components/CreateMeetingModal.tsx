    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Schedule New Meeting">
            <form onSubmit={handleSubmit} className="space-y-4">
                <input type="text" placeholder="Meeting Title" value={title} onChange={e => setTitle(e.target.value)} required className="w-full p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"/>
                <div className="flex gap-4">
                    <input type="date" value={date} onChange={e => setDate(e.target.value)} required className="w-full p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text" style={{colorScheme: 'light'}}/>
                    <input type="time" value={time} onChange={e => setTime(e.target.value)} required className="w-full p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text" style={{colorScheme: 'light'}}/>
                </div>
                
                <div>
                    <label className="block text-sm font-medium text-brand-text-dark mb-2">Select Attendees (Current Committee)</label>
                    <div className="max-h-40 overflow-y-auto space-y-2 p-3 bg-slate-100 rounded-md border border-slate-200">
                        {currentCommitteeMembers.map(member => (
                            <div key={member.id} className="flex items-center">
                                <input
                                    ...
                                    className="h-4 w-4 text-brand-accent bg-white border-slate-400 rounded focus:ring-brand-accent"
                                />
