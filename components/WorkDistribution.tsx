...
const TaskForm: React.FC<{
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <input name="name" value={formData.name} onChange={handleChange} placeholder="Task Name" required className="w-full p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"/>
      <div>
        <label className="text-sm text-brand-text-dark">Deadline</label>
        <input type="datetime-local" name="deadline" value={formData.deadline} onChange={handleChange} required className="w-full p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text" style={{colorScheme: 'light'}}/>
      </div>
      <div className="flex gap-4 items-center">
          <div className="flex-1">
            <label className="text-sm text-brand-text-dark">Workload (1-10): {formData.workload}</label>
            <input type="range" name="workload" min="1" max="10" value={formData.workload} onChange={handleChange} className="w-full h-2 bg-slate-300 rounded-lg appearance-none cursor-pointer accent-brand-accent"/>
          </div>
           <div className="w-24">
            <label className="text-sm text-brand-text-dark">Personnel</label>
            <input type="number" name="requiredPersonnel" min="1" value={formData.requiredPersonnel} onChange={handleChange} className="w-full p-2 rounded-md bg-white border border-slate-300"/>
          </div>
      </div>
        ...
        return (
            <div className="animate-fade-in space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {canEdit && (
                     <Card className="border-2 border-dashed border-slate-400 hover:border-brand-accent transition-colors flex flex-col items-center justify-center p-6 text-center">
                        ...
                        <select onChange={(e) => e.target.value && handleCreatePlan(e.target.value)} defaultValue="" className="w-full p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text">
        ...
        <div className="animate-fade-in space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className="lg:col-span-1 space-y-4">
                    <h2 className="text-2xl font-semibold text-brand-text border-b border-slate-200 pb-2">Configuration</h2>
                    <div>
                        <h3 className="text-lg font-semibold mb-2">Involved Committee Members</h3>
                        <div className="space-y-2 p-2 border border-slate-200 rounded-md">
                            {plan.involvedCommitteeMembers.map(memberId => {
                                const member = currentCommitteeMembers.find(m => m.id === memberId);
                                return <div key={memberId} className="flex items-center justify-between bg-slate-200 p-1.5 rounded-md"><span className="text-sm">{member?.name}</span>
                                {canEdit && <button onClick={() => handleMemberToggle(memberId)} className="text-red-500 text-xs font-bold">X</button>}
                                </div>
                            })}
                        </div>
                        {canEdit && (
                         <select onChange={(e) => e.target.value && handleMemberToggle(e.target.value)} defaultValue="" className="mt-2 w-full p-2 text-sm rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text">
                     ...
                     <div>
                        ...
                        {plan.tasks.map(task => (
                            <div key={task.id} className="p-2 bg-slate-200 rounded-md">
                        ...
                        {canEdit && <button onClick={() => { setEditingTask(null); setIsTaskModalOpen(true); }} className="mt-2 w-full text-sm bg-slate-200 hover:bg-slate-300 text-brand-text font-bold py-1.5 px-4 rounded-lg">Add New Task</button>}
                    </div>
                    ...
                    <Card>
                        ...
                        <div className="space-y-6">
                            {plan.involvedCommitteeMembers.map(memberId => {
                                ...
                                return (
                                <div key={memberId} className="bg-white p-4 rounded-lg border border-slate-200">
                ...
                <h2 className="text-2xl font-semibold text-brand-text mb-4">Deadline Timeline</h2>
                 <div className="flex overflow-x-auto space-x-4 pb-4">
                    {deadlines.length > 0 ? deadlines.map(({ date, tasks }) => (
                        <div key={date} className="flex-shrink-0 w-64 bg-white p-3 rounded-lg border border-slate-200">
                                ...
                                {tasks.sort((a,b) => new Date(a.deadline).getTime() - new Date(b.deadline).getTime()).map(task => (
                                    <div key={task.id} style={getTaskColor(task.id)} className="p-2 rounded-md border text-sm">
                                        ...
                                        <div className="flex gap-1 mt-1">
                                            {task.assignedTo.map(id => <span key={id} title={getCommitteeMemberById(id)?.name} className="w-5 h-5 rounded-full bg-slate-400 text-white text-xs flex items-center justify-center font-bold">{getCommitteeMemberById(id)?.name.charAt(0)}</span>)}
                                        </div>
                                    </div>
                                ))}
                            </div>
