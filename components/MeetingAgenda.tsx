...
const EditableAgendaItem: React.FC<{ 
    ...
    return (
        <li className={`flex items-center gap-3 p-3 bg-slate-100 rounded-lg border-2 ${focusClasses} transition-all duration-300`}>
            <input
                type="checkbox"
                checked={item.isCompleted}
                onChange={() => toggleAgendaItemStatus(meetingId, item.id)}
                disabled={!canEdit}
                className="w-5 h-5 text-brand-accent bg-white border-slate-400 rounded focus:ring-brand-accent disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
            />
            <div className="flex-1" onClick={() => !isEditing && canEdit && setIsEditing(true)}>
                {isEditing && canEdit ? (
                    <input
                        ...
                        autoFocus
                        className="w-full bg-white px-1 rounded-md border-b border-brand-accent focus:outline-none"
                    />
                ) : (
            ...
            {canEdit && (
                <button onClick={onFocus} className={`text-xs font-semibold py-1 px-2 rounded-md transition-colors ${isFocused ? 'bg-brand-accent text-white' : 'bg-slate-200 hover:bg-slate-300'}`}>
            ...
            <Card>
                ...
                <p className="text-brand-text-dark mt-1">
                 ...
                 <div className="mt-4 flex flex-wrap gap-2">
                    <span className="font-semibold text-sm mr-2">Attendees:</span>
                ...
                <div className="lg:col-span-3 space-y-4">
                    ...
                    {canEdit && (
                        <form onSubmit={handleAddAgendaItem} className="flex gap-2 pt-2">
                            <input type="text" value={newAgendaText} onChange={e => setNewAgendaText(e.target.value)} placeholder="Add new agenda item..." className="flex-1 p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"/>
                ...
                <div className="lg:col-span-2 space-y-6 lg:sticky lg:top-8">
                     <Card>
                        ...
                        <textarea
                            ...
                            rows={8}
                            className="w-full p-2 text-sm rounded-md bg-slate-100 border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"
                    ...
                    {meetingMinutes && (
                         <Card>
                             ...
                             <div className="prose prose-sm max-w-none bg-slate-50 p-3 rounded-md max-h-60 overflow-y-auto">
            ...
            <Modal isOpen={isMinutesModalOpen} onClose={() => setIsMinutesModalOpen(false)} title="File Meeting Minutes">
                <div className="space-y-4">
                    ...
                    <textarea
                        ...
                        rows={15}
                        className="w-full p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"
                        placeholder="### Title&#10;**Attendees:**..."
                    ></textarea>
