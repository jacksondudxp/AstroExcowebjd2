...
const WorkflowItem: React.FC<{ workflow: Workflow }> = ({ workflow }) => {
    ...
    return (
        <Card className="!p-0 overflow-hidden">
            <button onClick={handleToggle} className="w-full text-left p-4 flex justify-between items-center hover:bg-slate-50 transition-colors" aria-expanded={isExpanded}>
                <div>
                        ...
                        {workflow.tasks.map(task => (
                            <li key={task.id} className="flex items-center">
                                <input 
                                    ...
                                    className="w-5 h-5 text-brand-accent bg-slate-100 border-slate-300 rounded focus:ring-brand-accent"
                                />
