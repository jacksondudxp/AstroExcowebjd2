        ...
        <div className="mt-6">
            ...
            <form onSubmit={handleSubmit} className="flex gap-2 mt-4 pt-4 border-t border-slate-200">
                <input
                    ...
                    className="flex-1 p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"
                />
            ...
            <Card>
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <input
                        ...
                        className="w-full md:w-1/2 p-2 rounded-md bg-white border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"
                    />
                    <div className="flex flex-wrap gap-2">
                        <button onClick={() => setSelectedCategory('All')} className={`px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${selectedCategory === 'All' ? 'bg-brand-accent text-white' : 'bg-slate-200 hover:bg-slate-300 text-slate-700'}`}>All</button>
                        {KNOWLEDGE_CATEGORIES.map(category => (
                            <button key={category} onClick={() => setSelectedCategory(category)} className={`px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${selectedCategory === category ? 'bg-brand-accent text-white' : 'bg-slate-200 hover:bg-slate-300 text-slate-700'}`}>{category}</button>
                        ))}
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    {filteredArticles.length > 0 ? filteredArticles.map(article => (
                        <div key={article.id} className="bg-white rounded-lg overflow-hidden border border-slate-200">
                            <button onClick={() => toggleArticle(article.id)} className="w-full text-left p-4 flex justify-between items-center hover:bg-slate-50 transition-colors" aria-expanded={expandedArticleId === article.id}>
                                <div>
                                ...
                                <div className="p-4 border-t border-slate-200 animate-fade-in">
                                        ...
                                        <div className="mt-6">
                                            ...
                                            <div className="space-y-2">
                                                {article.attachments.map((file, index) => (
                                                    <a href={file.url} key={index} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-slate-100 p-2 rounded-md hover:bg-slate-200 transition-colors">
                                    ...
                                    {canEdit && (
                                        <div className="mt-6 pt-4 border-t border-slate-200 flex justify-end gap-3">
                                            <button onClick={() => handleOpenModal(article)} className="bg-slate-200 hover:bg-slate-300 text-brand-text font-semibold py-2 px-4 rounded-lg shadow-md transition-colors">Edit Article</button>
                                            <button onClick={() => handleDelete(article.id)} className="bg-red-100 hover:bg-red-200 text-red-700 font-semibold py-2 px-4 rounded-lg shadow-md transition-colors">Delete Article</button>
                                        </div>
                                    )}
