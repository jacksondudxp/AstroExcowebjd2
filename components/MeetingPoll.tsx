    ...
    return (
        <Card>
            <div className="flex justify-between items-start mb-3">
                ...
                {isHost && <button onClick={() => endPoll(meeting.id)} className="text-xs bg-red-100 text-red-700 hover:bg-red-200 font-semibold px-2 py-1 rounded-md">End Poll</button>}
            </div>
            ...
            <div className="space-y-3 mb-4">
                ...
                <PollResultBar label="Abstain" value={poll.options.abstain} total={totalVotes} color="bg-slate-400" />
            </div>

            {!hasVoted && (
                <div className="flex justify-around pt-3 border-t border-slate-200">
                    <button onClick={() => castVote(meeting.id, 'yes')} className="bg-green-100 hover:bg-green-200 text-green-800 font-bold py-2 px-4 rounded-lg">Vote Yes</button>
                    <button onClick={() => castVote(meeting.id, 'no')} className="bg-red-100 hover:bg-red-200 text-red-800 font-bold py-2 px-4 rounded-lg">Vote No</button>
                    <button onClick={() => castVote(meeting.id, 'abstain')} className="bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold py-2 px-4 rounded-lg">Abstain</button>
                </div>
            )}
             {hasVoted && (
                <p className="text-center text-sm font-semibold text-brand-accent pt-3 border-t border-slate-200">You have voted.</p>
            )}
        </Card>
    );
};
