    ...
    <div className="animate-fade-in space-y-6">
          ...
          <Card key={event.id} className="flex flex-col">
            ...
            {event.applicationsOpen && !event.isPast && (
                <div className="mt-4 pt-4 border-t border-slate-200">
                    <button onClick={() => setViewingApplicantsFor(event)} className="w-full text-center bg-slate-100 hover:bg-slate-200 text-brand-text font-semibold py-2 px-4 rounded-md transition-colors">
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="Create New Event">
        <form onSubmit={handleSubmit} className="space-y-4">
          <input type="text" name="title" placeholder="Event Title" value={newEvent.title} onChange={handleInputChange} required className="w-full p-2 rounded-md bg-slate-100 border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"/>
          <div className="flex gap-4">
            <input type="date" name="date" value={newEvent.date} onChange={handleInputChange} required className="w-full p-2 rounded-md bg-slate-100 border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text" style={{colorScheme: 'light'}}/>
            <input type="time" name="time" value={newEvent.time} onChange={handleInputChange} required className="w-full p-2 rounded-md bg-slate-100 border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text" style={{colorScheme: 'light'}}/>
          </div>
          <input type="text" name="location" placeholder="Location" value={newEvent.location} onChange={handleInputChange} required className="w-full p-2 rounded-md bg-slate-100 border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"/>
          <textarea name="description" placeholder="Description" value={newEvent.description} onChange={handleInputChange} required rows={4} className="w-full p-2 rounded-md bg-slate-100 border border-slate-300 focus:ring-2 focus:ring-brand-accent focus:outline-none text-brand-text"></textarea>
          <div className="flex items-center">
            <input 
                ...
                className="h-4 w-4 text-brand-accent bg-slate-200 border-slate-400 rounded focus:ring-brand-accent"
            />
