...
const CalendarPage: React.FC = () => {
    ...
    const renderHeader = () => {
        ...
        return (
            <div className="flex justify-between items-center mb-4 p-2">
                <button onClick={() => changeMonth(-1)} className="p-2 rounded-full hover:bg-slate-200 transition-colors" aria-label="Previous month">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                </button>
                <h2 className="text-2xl font-bold">{dateFormat.format(currentDate)}</h2>
                <button onClick={() => changeMonth(1)} className="p-2 rounded-full hover:bg-slate-200 transition-colors" aria-label="Next month">
    ...
    const renderDays = () => {
        ...
        return (
            <div className="grid grid-cols-7 text-center font-semibold text-brand-text-dark">
                {days.map(day => <div key={day} className="py-2 border-b-2 border-slate-300">{day}</div>)}
            </div>
        );
    };
    ...
    const renderCells = () => {
        ...
        while (day <= endDate) {
            ...
            cells.push(
                <div key={dayKey} className={`border-l border-t border-slate-300 h-36 flex flex-col p-1.5 ${isCurrentMonth ? 'bg-white' : 'bg-slate-100 text-brand-text-dark'}`}>
        ...
        return <div className="grid grid-cols-7 border-r border-b border-slate-300">{cells}</div>;
    };
